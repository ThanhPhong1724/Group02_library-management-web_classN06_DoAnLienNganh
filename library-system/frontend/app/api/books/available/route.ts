  ...
  try {
    ...
    if (q && q.trim()) {
      ...
      rows = await sql`
        ...
        WHERE (LOWER(b.tieu_de) LIKE LOWER(${searchTerm}) OR LOWER(b.tac_gia) LIKE LOWER(${searchTerm}))
        GROUP BY b.id, b.tieu_de, b.tac_gia
        HAVING COUNT(CASE WHEN c.trang_thai = 'available' THEN 1 END) > 0
        ORDER BY b.id ASC;
      `;
    } else {
      rows = await sql`
    ...
    console.error('Error in GET /api/books/available:', error);
